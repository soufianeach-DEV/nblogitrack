...
const ETATS = {
    TO_PAY: { libelle: 'À payer', classe: 'bg-brand-blue/10 text-brand-blue' },
...
function Encodage({ categories, vehicules, fournisseurs, onFermer }) {
    ...
    return (
        <form onSubmit={enregistrer} className="p-6">
            <h2 className="text-lg font-bold text-marine">Encoder une facture fournisseur</h2>

            <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                    <label htmlFor="fournisseur" className={intitule}>Fournisseur</label>
                    <input
                        id="fournisseur"
                ...
                <div>
                    <label htmlFor="reference" className={intitule}>Référence de la facture</label>
                    <input
                        id="reference"
                ...
                <div>
                    <label htmlFor="categorie" className={intitule}>Catégorie</label>
                ...
                <div>
                    <label htmlFor="vehicule" className={intitule}>Véhicule</label>
                    <select
                        id="vehicule"
                ...
                <div>
                    <label htmlFor="mois" className={intitule}>Période facturée</label>
                    <input
                        id="mois"
                        type="month"
                        value={data.mois}
                ...
                <div>
                    <label htmlFor="emission" className={intitule}>Émise le</label>
                    <input
                        id="emission"
                        type="date"
                ...
                {data.category === 'CARBURANT' ? (
                    <div>
                        <label htmlFor="litres" className={intitule}>Litres (vide pour GNC)</label>
                        <input
                            id="litres"
                            type="number"
                            step="0.01"
                    ...
                    <div>
                        <label htmlFor="km" className={intitule}>Kilomètres taxés</label>
                        <input
                            id="km"
                            type="number"
                            step="0.1"
                ...
                <div>
                    <label htmlFor="ht" className={intitule}>Montant hors TVA</label>
                    <input
                        id="ht"
                        type="number"
                        step="0.01"
                ...
                <div>
                    <label htmlFor="taux" className={intitule}>Taux de TVA</label>
                    <select id="taux" value={data.vat_rate} onChange={(e) => setData('vat_rate', e.target.value)} className={champ}>
                        <option value="0">0 % — hors champ ou exonéré</option>
                ...
                <label className="flex items-end gap-2 pb-2">
                    ...
                    />
                    <span className="text-sm text-slate-600">TVA déductible</span>
                </label>
            </div>

            <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
                <p className="text-sm text-slate-600">
                    TVA {euros(tva)} — <span className="font-bold text-marine">TTC {euros(ht + tva)}</span>
                </p>
            ...
            header={
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="text-sm text-slate-600">Les factures des fournisseurs, rattachées au parc.</p>
                    </div>
                    <button
                        type="button"
        ...
        >
            ...
            <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-2xl bg-white p-5 shadow-sm">
                    <p className="text-xs uppercase tracking-wide text-slate-600">Reste à payer</p>
                ...
                <div className="rounded-2xl bg-white p-5 shadow-sm">
                    <p className="text-xs uppercase tracking-wide text-slate-600">TVA déductible cumulée</p>
            ...
            <div className="mt-4">
                <BarreFiltres
                    ...
                    filtres={filtres}
                    placeholder="Fournisseur, référence, immatriculation…"
                    listes={[{
                        champ: 'categorie',
                        intitule: 'Toutes les catégories',
                        options: Object.entries(categories).map(([valeur, libelle]) => ({ valeur, libelle })),
                    }]}
                    compteurs={[
                        { libelle: 'Toutes', valeur: null, nombre: compteurs.total },
                        { libelle: 'À payer', valeur: 'a_payer', nombre: compteurs.a_payer },
                        { libelle: 'En retard', valeur: 'retard', nombre: compteurs.retard, alerte: true },
                        { libelle: 'Payées', valeur: 'payees', nombre: compteurs.payees },
                    ]}
                />
            </div>
                    ...
                    <table className="min-w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-100 text-left text-xs uppercase tracking-wide text-slate-600">
                                <th scope="col" className="whitespace-nowrap px-4 py-3 font-semibold">Fournisseur</th>
                                <th scope="col" className="whitespace-nowrap px-4 py-3 font-semibold">Catégorie</th>
                                ...
                                <th scope="col" className="whitespace-nowrap px-4 py-3 text-right font-semibold">HT</th>
                                <th scope="col" className="whitespace-nowrap px-4 py-3 text-right font-semibold">TVA</th>
                                <th scope="col" className="whitespace-nowrap px-4 py-3 text-right font-semibold">TTC</th>
                                ...
                                return (
                                    <tr key={a.id} className="border-b border-slate-50 last:border-0">
                                        ...
                                        <td className="whitespace-nowrap px-4 py-3 text-slate-600">
                                            {a.categorie}
                                            {a.litres !== null && (
                                                <span className="block text-xs">{a.litres.toLocaleString('fr-FR')} l</span>
                                            )}
                                            {a.km_taxes !== null && (
                                                <span className="block text-xs">{a.km_taxes.toLocaleString('fr-FR')} km taxés</span>
                                            )}
                                        </td>
            ...
            {achats.last_page > 1 && (
                <div className="mt-4 flex items-center justify-between text-sm text-slate-600">
                    <span>Page {achats.current_page} sur {achats.last_page} — {achats.total} factures</span>
