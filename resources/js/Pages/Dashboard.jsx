...
function StatCard({ label, value, icon, accent }) {
    return (
        <div className="rounded-2xl bg-white p-6 shadow-sm">
            <span className={`material-symbols-outlined rounded-lg p-2 ${accent}`}>{icon}</span>
        ...
        <AuthenticatedLayout
            header={
                <div>
                    ...
                    <p className="text-sm text-slate-500">Aperçu de vos ordres de transport.</p>
                </div>
            }
        >
            ...
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                <StatCard label="Total ordres" value={stats.total} icon="local_shipping" accent="bg-marine/10 text-marine" />
                <StatCard label="En attente" value={stats.pending} icon="schedule" accent="bg-status-pending/10 text-status-pending" />
                <StatCard label="En cours" value={stats.in_progress} icon="sync" accent="bg-status-progress/10 text-status-progress" />
                <StatCard label="Livrés" value={stats.delivered} icon="check_circle" accent="bg-status-delivered/10 text-status-delivered" />
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                        <tbody>
                            ...
                            {recent.length === 0 && (
                                <tr><td className="px-6 py-8 text-center text-slate-400" colSpan="4">Aucun ordre pour l'instant.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
