...
@php($titre = $facture->estAvoir()
    ? $t::t('courriel.avoir_titre', 'Votre avoir :reference', ['reference' => $facture->reference])
    : $t::t('courriel.facture_titre', 'Votre facture :reference', ['reference' => $facture->reference]))

@section('titre', $titre)

@section('contenu')
    <h1 style="margin:0 0 8px; color:#14324F; font-size:22px;">{{ $titre }}</h1>
    <p style="margin:0 0 24px; color:#1A202C; font-size:14px; line-height:1.6;">
        {{ $t::t('courriel.bonjour', 'Bonjour :prenom,', ['prenom' => $prenom]) }}<br>
        @if ($facture->estAvoir())
        {{ $t::t('courriel.avoir_texte', 'voici l\'avoir de :montant qui annule la facture :facture. Vous le trouverez en pièce jointe au format PDF et au format XML Peppol. Rien n\'est à payer pour cette facture.', [
            'montant' => $f::montant($facture->amount_incl_tax),
            'facture' => $facture->creditedInvoice?->reference,
        ]) }}
        @else
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#F5F7FA; border-radius:10px; margin-bottom:24px;">
        <tr>
            <td style="padding:20px 24px;">
                ...
                <div style="color:#14324F; font-size:22px; font-weight:bold; margin:4px 0 12px;">{{ $f::montant($facture->solde()) }}</div>
